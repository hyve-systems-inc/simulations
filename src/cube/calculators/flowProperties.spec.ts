...
describe("Flow Property Calculations", () => {
  ...
  describe("Flow Distribution", () => {
    ...
    it("varies with height", () => {
      const turbulence = 0.1; // Moderate turbulence intensity
      const positions = [
      ...
      const distributions = positions.map((pos) =>
        flow.calculateFlowDistribution(pos, turbulence, testConfig)
      );
    ...
    /**
     * Test turbulence enhancement of flow distribution
     * Reference: Section IV, 4.3 - Turbulence enhancement factor
     *
     * Physical basis for turbulence effects:
     * - Base turbulence (I = 0.1):
     *   - Typical for developed duct flow
     *   - Re ≈ 20,000
     *   - Base mixing level
     *
     * - Enhanced turbulence (I = 0.2):
     *   - Generated by:
     *     - Product obstacles
     *     - Flow separation
     *     - Geometric transitions
     *   - Re ≈ 10,000 (higher relative fluctuations)
     *
     * Enhancement mechanism:
     * - Increased mixing
     * - Better penetration into product bulk
     * - Reduced flow channeling
     * - Enhanced heat/mass transfer (h ∝ I^0.5)
     *
     * Expected enhancement:
     * Distribution ∝ (1 + 0.2*I) from experimental correlations
     */
    it("is enhanced by turbulence", () => {
      const position = new Position(1, 1, 1);
      const lowTurb = flow.calculateFlowDistribution(position, 0.1, testConfig);
      const highTurb = flow.calculateFlowDistribution(
        position,
        0.2,
        testConfig
      );

      expect(highTurb).to.be.greaterThan(lowTurb);

      // Check enhancement factor
      const enhancement = highTurb / lowTurb;
      const expectedEnhancement = (1 + 0.2 * 0.2) / (1 + 0.2 * 0.1);
      expect(enhancement).to.be.approximately(expectedEnhancement, 0.01);
    });
    ...
    it("is lower at edges", () => {
      ...
      const centerDist = flow.calculateFlowDistribution(
        centerPos,
        turbulence,
        testConfig
      );
      const edgeDist = flow.calculateFlowDistribution(
        edgePos,
        turbulence,
        testConfig
      );
